...
static NFCSTATUS phLibNfc_InventoryCmd(void *pContext, NFCSTATUS status, void *pInfo);
static NFCSTATUS phLibNfc_InventoryResp(void *pcontext, NFCSTATUS status, void *pInfo);
...
static NFCSTATUS phLibNfc_GetTagInfoComplete(void *pContext,NFCSTATUS status,void *pInfo);

phLibNfc_Sequence_t gphLibNfc_Iso15693GetTagInfoSeq[] = {
    {&phLibNfc_InventoryCmd, &phLibNfc_InventoryResp},
    {&phLibNfc_GetSysInfoCmd, &phLibNfc_GetSysInfoResp},
    {NULL, &phLibNfc_GetTagInfoComplete}
};
...
static NFCSTATUS phLibNfc_InventoryCmd(void *pContext,NFCSTATUS status,void *pInfo)
{
    ...
    uint8_t aGetSysInfoCmdBuff[5] = {0};
    uint32_t dwIndex = 0;
    ...
    if(NULL != pCtx)
    {
            ...
            (phNciNfc_eISO15693_PICC == pDevInfo->pRemDevList[dwIndex]->RemDevType))
        {
            ...
            // Request flags
            aGetSysInfoCmdBuff[0] = 0x26;

            // Command code
            aGetSysInfoCmdBuff[1] = 0x01;

            // Mask length
            aGetSysInfoCmdBuff[2] = 0x00;

            tIso15693Info.tSendData.wLen = 3;
    ...
    if(NULL != pLibContext)
    {
        ...
        if(NFCSTATUS_SUCCESS == status)
        {
                ...
                (pRecvData->wLen >= PHLIBNFC_GETSYSINFO_RESP_MINLEN))
            {
                ...
                {
                    PH_LOG_LIBNFC_INFO_STR("Updating ISO15693 Inventory info");
                    (void)phNciNfc_Update15693InventoryInfo(pLibContext->sHwReference.pNciHandle,
                                                            pDeviceInfo->pRemDevList[0],
                                                            pRecvData->pBuff);
                }
                else
                {
                    PH_LOG_LIBNFC_WARN_STR("Error_flag is set in the response.");
                }
            }
            else
            {
