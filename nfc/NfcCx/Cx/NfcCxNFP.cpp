...
Routine Description:
    ...
    This routine looks at the subscription lists
    to determine if there is a discovery client available
...
Return Value:
    ...
    TRUE - A discovery publication/subsctiption is active
    FALSE - A discovery publication/subsctiption isn't active

Remarks:

    This function shouldn't take either power policy or state lock otherwise
    it would lead to deadlocks since we call into the RF module with 
    taking these locks and so acquiring these here would deadlock.
    Here we peak at data to configure polling loop and hence should is safe
...
--*/
{
    ...
    for (ple = NfpInterface->SubsList.Flink;
         ple != &NfpInterface->SubsList && FALSE == fEnabled;
         ple = ple->Flink) {
        PNFCCX_FILE_CONTEXT subClient = CONTAINING_RECORD(ple, NFCCX_FILE_CONTEXT, ListEntry);
        fEnabled = subClient->Enabled;
    }

    for (ple = NfpInterface->ArrivalSubsList.Flink;
         ple != &NfpInterface->ArrivalSubsList && FALSE == fEnabled;
         ple = ple->Flink) {
        PNFCCX_FILE_CONTEXT subClient = CONTAINING_RECORD(ple, NFCCX_FILE_CONTEXT, ListEntry);
        fEnabled = subClient->Enabled;
    }

    for (ple = NfpInterface->RemovalSubsList.Flink;
         ple != &NfpInterface->RemovalSubsList && FALSE == fEnabled;
         ple = ple->Flink) {
        PNFCCX_FILE_CONTEXT subClient = CONTAINING_RECORD(ple, NFCCX_FILE_CONTEXT, ListEntry);
        fEnabled = subClient->Enabled;
    }
