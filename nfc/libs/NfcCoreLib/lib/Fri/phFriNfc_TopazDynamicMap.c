...
#include "phFriNfc_TopazDynamicMap.tmh"
...
#define TOPAZ_INCREMENT_SKIP_STATIC_BLOCK(block_no) \
    ((((block_no) + (uint8_t)1) == (uint8_t)TOPAZ_STATIC_LOCK_FIRST_BLOCK_NO) ? \
    (((block_no) + (uint8_t)1) + (uint8_t)TOPAZ_STATIC_LOCK_BLOCK_AREAS) : \
    ((block_no) + (uint8_t)1))
...
NFCSTATUS  phFriNfc_TopazDynamicMap_ChkNdef( phFriNfc_NdefMap_t     *NdefMap)
{
         ...
         NdefMap->SendRecvBuf != NULL)
    {
        ...
        (void)phOsalNfc_SetMemory ((void *)&(NdefMap->LockTlv), 0,
                        sizeof (phFriNfc_LockCntrlTLVCont_t));

        (void)phOsalNfc_SetMemory ((void *)&(NdefMap->MemTlv), 0,
        ...
        *NdefMap->SendRecvBuf = PH_FRINFC_TOPAZ_CMD_RSEG;
        ...
        Result = phFriNfc_Tpz_H_Read(NdefMap);
    }
                                        ...
                                        uint8_t                         Offset)
{
    ...
    *NdefMap->NumOfBytesRead = 0;
        ...
        (PH_FRINFC_NDEFMAP_READ_OPE != NdefMap->PrevOperation))
    {
        ...
        NdefMap->State = PH_FRINFC_TOPAZ_STATE_READ;
        NdefMap->SendRecvBuf[0] = PH_FRINFC_TOPAZ_CMD_RSEG;
        Result = phFriNfc_Tpz_H_Read(NdefMap);
    }
    else
    {
    ...
    phFriNfc_NdefMap_t     *psNdefMap)
{
    ...
    *psNdefMap->SendRecvBuf = (uint8_t)PH_FRINFC_TOPAZ_CMD_WRITE_1E;
    ...
    result = phFriNfc_Tpz_H_Write (psNdefMap, &cc_read_only_byte,
                                    1);
    ...
    phFriNfc_NdefMap_t          *psNdefMap)
{
    ...
    (void)phOsalNfc_MemCopy ((void *)lock_bytes_value, (void *)psNdefMap->SendRecvBuf,
    ...
    *psNdefMap->SendRecvBuf = PH_FRINFC_TOPAZ_CMD_WRITE_E8;
    ...
    result = phFriNfc_Tpz_H_Write (psNdefMap, lock_bytes_value,
    ...
    switch (e_readonly_seq)
    {
        case WR_READONLY_CC:
        {
            if (TOPAZ_WRITE_1_RESPONSE == *psNdefMap->SendRecvLength)
            {
                ...
                *psNdefMap->SendRecvBuf = PH_FRINFC_TOPAZ_CMD_READ8;
                ...
                /* Call read segment */
                result = phFriNfc_Tpz_H_Read (psNdefMap);
            }
            else
            {
                result = PHNFCSTVAL(CID_FRI_NFC_NDEF_MAP,
        ...
        case RD_STATIC_LOCK_BYTE0:
        {
            if (TOPAZ_READ_8_RESPONSE == *psNdefMap->SendRecvLength)
            {
                ...
                (void)phOsalNfc_MemCopy ((void *)static_lock_bytes,
                ...
                *psNdefMap->SendRecvBuf = (uint8_t)PH_FRINFC_TOPAZ_CMD_WRITE_1E;
                ...
                result = phFriNfc_Tpz_H_Write (psNdefMap, &lock_byte_value,
                                                    1);
        ...
        case WR_NDEF_T_TLV:
        {
            ...
            if (TOPAZ_WRITE_8_RESPONSE == *psNdefMap->SendRecvLength)
            {
                ...
                *psNdefMap->SendRecvBuf = PH_FRINFC_TOPAZ_CMD_WRITE_1E;
                ...
                result = phFriNfc_Tpz_H_Write (psNdefMap, write_buf,
                                                sizeof (write_buf));
            }
            else
            {
    ...
    phFriNfc_NdefMap_t          *psNdefMap)
{
    ...
    (void)phOsalNfc_MemCopy ((void *)write_buf, (void *)
                psNdefMap->SendRecvBuf, TOPAZ_WRITE_8_DATA_LENGTH);

    /* Update the TYPE field of the NDEF TLV */
    if (ps_tpz_info->ByteNumber >= TOPAZ_BYTES_PER_BLOCK)
    {
        PH_LOG_FRI_CRIT_STR("Invalid ByteNumber!");
        return NFCSTATUS_INVALID_PARAMETER;
    }

    write_buf[ps_tpz_info->ByteNumber] = PH_FRINFC_TOPAZ_TLV_NDEF_T;
    ...
    phFriNfc_NdefMap_t          *psNdefMap)
{
    ...
    psNdefMap->psDepAdditionalInfo.NodeAddress = PH_FRINFC_TOPAZ_VAL0;
    ...
    *psNdefMap->SendRecvLength = psNdefMap->TempReceiveLength;
    ...
    psNdefMap->Cmd.JewelCmd = phNfc_eJewel_Raw;
    ...
    *psNdefMap->SendRecvLength = psNdefMap->TempReceiveLength;
    ...
    if (NFCSTATUS_SUCCESS == result)
    {
        ...
        (void)phOsalNfc_MemCopy ((void *)(psNdefMap->SendRecvBuf + send_index),
                    (void *)psNdefMap->psRemoteDevInfo->RemoteDevInfo.Jewel_Info.Uid,
                    TOPAZ_UID_LENGTH);
        send_index = (uint8_t)(send_index + TOPAZ_UID_LENGTH);
    ...
    phFriNfc_NdefMap_t          *psNdefMap)
{
    ...
    switch (psNdefMap->PrevOperation)
    {
        ...
        case  PH_FRINFC_NDEFMAP_WRITE_OPE:
        {
            ...
            if(TOPAZ_READ_8_RESPONSE == *psNdefMap->SendRecvLength)
            {
                (void)phOsalNfc_MemCopy ((void *)ps_tpz_info->CCByteBuf,
                            (void *)(psNdefMap->SendRecvBuf),
                ...
                if (NFCSTATUS_SUCCESS == result)
                {
                    ...
                    else
                    {
                        ...
                        *psNdefMap->SendRecvBuf = PH_FRINFC_TOPAZ_CMD_WRITE_1E;
                        ...
                        /* Call read 8 */
                        result = phFriNfc_Tpz_H_Write (psNdefMap, write_buffer,
                                                    sizeof (write_buffer));
                    }
    ...
    phFriNfc_NdefMap_t  *psNdefMap)
{
    ...
    {
        switch (psNdefMap->TopazContainer.CCByteBuf[3])
        {
            ...
            default:
            {
                ...
                result = PHNFCSTVAL(CID_FRI_NFC_NDEF_MAP,
                                    NFCSTATUS_NO_NDEF_SUPPORT);
                PH_LOG_NDEF_WARN_STR("CC byte 3 did not equal the expected read and write access capabilities. CC byte 3 = 0x%02X",
                                     psNdefMap->TopazContainer.CCByteBuf[3]);
                break;
            }
        }
    }
    ...
    phFriNfc_NdefMap_t          *psNdefMap)
{
    ...
    phOsalNfc_MemCopy(ps_tpz_info->CCByteBuf,
    ...
    /* 1st CC byte value is NDEF magic number = 0xE1 */
    ...
    else
    {
        result = PHNFCSTVAL(CID_FRI_NFC_NDEF_MAP,
                            NFCSTATUS_NO_NDEF_SUPPORT);
        PH_LOG_NDEF_WARN_STR("CC byte 0 did not equal NDEF magic number. CC byte 0 = 0x%02X",
                             p_recv_buf[parse_index]);
    }
    ...
    if (NFCSTATUS_SUCCESS == result)
    {
        ...
        else
        {
            result = PHNFCSTVAL(CID_FRI_NFC_NDEF_MAP,
                                NFCSTATUS_NO_NDEF_SUPPORT);
            PH_LOG_NDEF_WARN_STR("CC byte 2 did not equal expected memory size for dynamic Topaz card. CC byte 2 = 0x%02X",
                                 p_recv_buf[parse_index]);
        }
    }
    ...
    phFriNfc_NdefMap_t          *psNdefMap)
{
        ...
        (PH_FRINFC_TOPAZ_CC_READWRITE != ps_tpz_info->CCByteBuf[3]))
    {
        result = PHNFCSTVAL(CID_FRI_NFC_NDEF_MAP,
                            NFCSTATUS_NO_NDEF_SUPPORT);
        PH_LOG_NDEF_WARN_STR("CC bytes did not equal expected CC bytes in order to write to tag");
    }
    ...
    phFriNfc_NdefMap_t          *psNdefMap)
{
    ...
    if (0 != ps_tpz_info->ReadBufferSize)
    {
            ...
            ps_tpz_info->ReadBufferSize)
        {
            ...
            (void)phOsalNfc_MemCopy ((void *)(psNdefMap->ApduBuffer + psNdefMap->ApduBuffIndex),
        ...
        else
        {
            ...
            if(copy_length > ARRAYSIZE(copy_temp_buf))
            {
                PH_LOG_FRI_CRIT_STR("Temp buffer too small! "
                                    "ApduBufferSize: %u "
                                    "ApduBuffIndex: %hu "
                                    "ReadBufferSize: %u",
                                    psNdefMap->ApduBufferSize,
                                    psNdefMap->ApduBuffIndex,
                                    (uint32_t)(ps_tpz_info->ReadBufferSize));
            ...
            (void)phOsalNfc_MemCopy ((void *)copy_temp_buf,
                    (void *)(ps_tpz_info->ReadBuffer + read_copy_length),
                    copy_length);

            /* Copy data from temporary buffer to " ReadBuffer " */
            (void)phOsalNfc_MemCopy ((void *)ps_tpz_info->ReadBuffer,
    ...
    while (copy_index < recv_length)
    {
        ...
        if (0 != copy_length)
        {
            ...
            if (exact_copy_length != psNdefMap->ApduBuffIndex)
            {
                ...
                psNdefMap->ApduBuffIndex = (uint16_t)(psNdefMap->ApduBuffIndex
                                            + copy_length);
                ps_tpz_info->RemainingReadSize = (uint16_t)(ps_tpz_info->RemainingReadSize
                                            - copy_length);
            }
            else
            {
        ...
        else
        {
            switch (expected_seq)
            {
                ...
                case LOCK_V_TLV:
                {
                    ...
                    {
                        (void)phOsalNfc_MemCopy ((void *)psNdefMap->LockTlv.LockTlvBuff,
    ...
    phFriNfc_NdefMap_t          *psNdefMap)
{
    ...
    (void)phOsalNfc_MemCopy ((void *)write_buf, (void *)psNdefMap->SendRecvBuf,
    ...
    else
    {
        *psNdefMap->SendRecvBuf = PH_FRINFC_TOPAZ_CMD_WRITE_E8;
        ...
        if ((write_len - psNdefMap->ApduBuffIndex) >= (uint16_t)TOPAZ_WRITE_8_DATA_LENGTH)
        {
            ...
            (void)phOsalNfc_MemCopy ((void *)write_buf,
        ...
        else
        {
            ...
            (void)phOsalNfc_SetMemory((void *)(write_buf + copy_length), 0x00,
    ...
    phFriNfc_Tpz_ParseSeq_t     *seq_to_execute)
{
    ...
    {
        case PH_FRINFC_TOPAZ_TLV_LOCK_CTRL_T:
        {
    ...
    phFriNfc_Tpz_ParseSeq_t     *seq_to_execute)
{
    ...
    switch (p_parse_data[parse_index])
    {
        ...
        case PH_FRINFC_TOPAZ_TLV_MEM_CTRL_T:
        {
    ...
    uint8_t                     *p_lock_info)
{
    ...
    */
    ...
    if (
        ...
        (((ps_locktlv_info->ByteAddr + ps_locktlv_info->Size - (uint8_t)1) >=
        (uint16_t)TOPAZ_STATIC_LOCK_RES_START) &&
        ((ps_locktlv_info->ByteAddr + ps_locktlv_info->Size - (uint8_t)1) <
        (uint16_t)TOPAZ_STATIC_LOCK_RES_END))
        )
    {
    ...
    uint8_t                     *p_mem_info)
{
    ...
    */
    ...
    if (
        ...
        (((ps_memtlv_info->ByteAddr + ps_memtlv_info->Size - (uint8_t)1) >=
        (uint16_t)TOPAZ_STATIC_LOCK_RES_START) &&
        ((ps_memtlv_info->ByteAddr + ps_memtlv_info->Size - (uint8_t)1) <
        (uint16_t)TOPAZ_STATIC_LOCK_RES_END)) ||
