...
static
void
phFriNfc_MifStd1k_H_BlkChk(
    _Inout_ phFriNfc_NdefMap_t  *NdefMap,
    _In_    uint8_t             SectorID,
    _Inout_ uint8_t             *callbreak
    );
            ...
            case PH_FRINFC_NDEFMAP_STATE_WR_NDEF_LEN:
                ...
                else
                {
                    ...
                    if(NdefMap->StdMifareContainer.TempBlockNo ==
                        NdefMap->StdMifareContainer.currentBlock)
                    {
                        (void)phOsalNfc_MemCopy( NdefMap->StdMifareContainer.internalBuf,
...
static NFCSTATUS phFriNfc_MifStd_H_WrABlock(phFriNfc_NdefMap_t *NdefMap)
{
        ...
        (uint16_t)NdefMap->ApduBufferSize)
    {
        ...
        if(NdefMap->StdMifareContainer.internalLength > 0)
        {
            ...
            (void)phOsalNfc_MemCopy(&( NdefMap->SendRecvBuf[Temp16Bytes]),
        ...
        else
        {
            if(RemainingBytes >= (MIFARE_MAX_SEND_BUF_TO_WRITE - Temp16Bytes))
            {
                ...
                (void)phOsalNfc_MemCopy( &(NdefMap->SendRecvBuf[
                        Temp16Bytes]),
                        &(NdefMap->ApduBuffer[
                        NdefMap->ApduBuffIndex]),
                        (MIFARE_MAX_SEND_BUF_TO_WRITE - Temp16Bytes));
        ...
        (void)phOsalNfc_MemCopy( NdefMap->StdMifareContainer.Buffer,
...
static NFCSTATUS phFriNfc_MifStd_H_ProBytesToWr(phFriNfc_NdefMap_t        *NdefMap)
{
    ...
    if(*NdefMap->SendRecvLength == PH_FRINFC_MIFARESTD_BYTES_READ)
    {
        (void)phOsalNfc_MemCopy(&NdefMap->SendRecvBuf[PH_FRINFC_MIFARESTD_VAL1],
                    NdefMap->SendRecvBuf,
                    PH_FRINFC_MIFARESTD_BLOCK_BYTES);
               ...
               const uint8_t *ScrtKeyB)
{
    ...
    else
    {
        ...
        if(ScrtKeyB == NULL)
        {
            phOsalNfc_SetMemory(NdefMap->StdMifareContainer.UserScrtKeyB, PH_FRINFC_MFSTD_FMT_DEFAULT_KEY,
                    PH_FRINFC_MIFARESTD_KEY_LEN);
        }
        else
        {
            phOsalNfc_MemCopy(NdefMap->StdMifareContainer.UserScrtKeyB, ScrtKeyB, PH_FRINFC_MIFARESTD_KEY_LEN);
        }
